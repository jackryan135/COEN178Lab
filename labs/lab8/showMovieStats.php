...
<head>
   ...
   <!-- Edit the link -->
   <a href='http://linux.students.engr.scu.edu/~jryan/showMovieStats.php?show=true'>Show Movie Stats</a>
   <br><br>
<?php
...
function showStats(){
	...
	$conn = oci_connect('jryan', 'helloworld1', '//dbserver.engr.scu.edu/db11g');
	if (!$conn) {
		$e = oci_error();
    ...
    $query2 = oci_parse($conn, "SELECT movieName, NoOfReviews FROM (SELECT movieName, Count(*) NoOfReviews FROM MovieReviews GROUP BY movieName Order By NoOfReviews DESC) WHERE ROWNUM = 1");

	oci_execute($query2);
    
    while (( $row = oci_fetch_array($query2, OCI_BOTH)) != false) {
        echo "<h1> <font color='blue'> Movie(s) with most no. of reviews </font></h1></br>";
        echo "<font color='black'> Movie: </font>";
        echo "<font color='green'> $row[0] </font>";
        echo "<font color='black'> No. of Reviews: </font> <font color='red'> $row[1]</font></br>";
    }
